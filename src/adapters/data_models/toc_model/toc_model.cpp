...
TOCModel::TOCModel(fz_outline* outline, IDocumentAccess* documentAccess,
                   QObject* parent) :
    QAbstractItemModel(parent),
    m_documentAccess(documentAccess)
{
...
TOCItem* TOCModel::getTOCItemFromOutline(fz_outline* outline)
{
    ...
    if(location.page == -1)
    {
        auto docLocation = m_documentAccess->getPositionOfLink(outline->uri);
        pageNumber = docLocation.pageNumber;
        yOffset = docLocation.yOffset;
    }
    else
    {
        pageNumber = m_documentAccess->getPageNumberFromLocation(&location);
    }
